...
def _write_entry_shim(project_root: Path) -> Path:
    """Write a tiny launcher PyInstaller can analyze (packages with relative
    imports cannot be used as a PyInstaller entry script directly)."""
    shim = project_root / "build" / "_pyinstaller_entry.py"
    shim.parent.mkdir(exist_ok=True)
    shim.write_text(
        "\n".join(
            [
                '"""PyInstaller entry shim (generated by scripts/build_binary.py)."""',
                "import sys",
                "",
                "from opencode_rate_limiter import main",
                "",
...
def main() -> int:
    ...
    # Written after the cleanup above so it is not wiped by it
    src = _write_entry_shim(project_root)
