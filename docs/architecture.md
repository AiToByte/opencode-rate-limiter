...
版本：0.7.0（2026-09） · 适用代码：`opencode_rate_limiter/` 包（当前 main）
...
| 决策 | 备选 | 选择与理由 |
|------|------|-----------|
| 代码组织：单文件 → 包 | 继续单文件（分发简单） | **拆为 14 模块**（`errors` 为统一错误分类层）。2500 行单文件的 patch 边界、测试隔离、职责演化都已到极限；`__init__` 再导出保证 API 与入口零破坏 |
